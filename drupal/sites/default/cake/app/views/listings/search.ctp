...
//	foreach($cities as $key => $value){
	foreach($cities_new as $key => $value){
		...
		echo '<fieldset>';
		echo '<legend>';
		echo $html->link($key, array('controller' => 'listings', 'action' => 'region' , Inflector::slug(strtolower($key))));
		...
		foreach($cities[$key] as $city){
			...
			echo '<div class="checkbox"><input name="data[Listing][city][]" '.$checked_status.' value="'.$val.'" id="ListingCity'.$val.'" type="checkbox">'.$html->link($city, array('controller' => 'listings', 'action' => 'city', Inflector::slug(strtolower($city)))).' ('.$city_count[$city].') </div>';
...
/* 		echo $form->submit('Search'); */
	...
	//echo '<p>'.$html->link('List of All Regions', array('controller' => 'listings', 'action' => 'all')).'</p>';
?>